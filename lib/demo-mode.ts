...
import staffPicksData from '@/data/mock/staff-picks.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'staff-picks.json': staffPicksData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetBooks') || query.includes('nodeBooks')) {
      return loadMockData('books.json')
    }
    ...
    if (query.includes('GetStaffPicks') || query.includes('nodeStaffPicks')) {
      return loadMockData('staff-picks.json')
    }
