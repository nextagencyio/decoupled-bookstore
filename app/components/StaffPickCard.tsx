...
interface StaffPickCardProps {
  item: DrupalStaffPick
}

export default function StaffPickCard({ item }: StaffPickCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
        {(item as any).bookTitle && (
          <p className="text-sm text-accent-700 font-medium mb-2 uppercase tracking-wider">{(item as any).bookTitle}</p>
        )}
